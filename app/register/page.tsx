...
export default function RegisterPage() {
  ...
  const handleDigiLockerAuth = async () => {
    setLoading(true)
    try {
      const response = await fetch('http://localhost:5000/api/digilocker/initiate', {
        method: 'POST',
        headers: {
  ...
  const handleDigiLockerConsent = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate DigiLocker consent (for sandbox)
      const consentResponse = await fetch('http://localhost:5000/api/digilocker/simulate-consent', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (consentResponse.ok && consentData.success) {
        // Register user with DigiLocker data
        const registerResponse = await fetch('http://localhost:5000/api/register-with-digilocker', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
